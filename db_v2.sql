...
-- Create indices for schedule lookups
...
------------------------------------------
-- Live Sessions and Attendance Tables
------------------------------------------

-- Live Sessions table (referenced by realtime extensions)
CREATE TABLE live_sessions (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    course_id UUID NOT NULL REFERENCES courses(id),
    group_ids UUID[] NOT NULL, -- Array of group IDs for multi-group sessions
    teacher_id UUID NOT NULL REFERENCES teacher_profiles(id),
    title VARCHAR(100),
...
-- Attendance records table (referenced by realtime extensions)
...
-- Create a view for student timeline
...
-- Create a view for teacher timeline
...
-- Function to generate sessions from weekly schedule
...
CREATE PUBLICATION live_attendance_pub FOR TABLE
